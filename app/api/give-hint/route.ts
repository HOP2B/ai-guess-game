...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check forbidden words for this character
    const forbiddenWords = await prisma.forbiddenWord.findMany({
      where: { characterId: game.characterId },
      select: { word: true },
    });
    ...
    const prompt = `You are playing a character guessing game. The possible characters are: ${characterNames}.
...
Guess which character it is. Respond with only the character name, nothing else. If you're not sure, make your best guess.`;
    ...
    // Check if the guess is correct (case-insensitive, trim whitespace)
    const normalizedGuess = guess?.toLowerCase().trim();
    const normalizedName = game.character.name.toLowerCase().trim();
    ...
    if (isCorrect) {
      ...
      await prisma.$transaction([
        ...
        prisma.user.update({
          ...
          data: { score: { increment: game.character.points } },
        }),
      ]);
    }
    ...
    return NextResponse.json({
      guess,
      isCorrect,
      correctCharacter: isCorrect ? game.character.name : undefined
    });
  } catch (error) {
